...
def _kit_framework_tools() -> Path:
    """Retourne le chemin ``grimoire-kit/framework/tools/`` depuis ce module."""
    here = Path(__file__).resolve()
    return here.parent.parent.parent.parent / "framework" / "tools"


def _load_context_router():
    """Charge le module context_router (préfère l'import kit canonique)."""
    mod_name = "context_router"
    if mod_name in sys.modules:
        return sys.modules[mod_name]
    try:
        from grimoire.tools import context_router as mod  # type: ignore

        sys.modules[mod_name] = mod
        return mod
    except Exception:
        pass
    router_path = _kit_framework_tools() / "context-router.py"
...
def _load_context_summarizer():
    """Charge le module context_summarizer (legacy path framework/tools/)."""
    mod_name = "context_summarizer"
    if mod_name in sys.modules:
        return sys.modules[mod_name]
    summarizer_path = _kit_framework_tools() / "context-summarizer.py"
