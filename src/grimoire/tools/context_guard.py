"""Context Guard — token budget analyzer for Grimoire agents.
...
def resolve_agent_loads(agent_path: Path, project_root: Path) -> list[FileLoad]:
    ...
    # Shared context
    shared = project_root / "_grimoire/_memory/shared-context.md"
    if shared.exists():
        loads.append(FileLoad(
            path="_grimoire/_memory/shared-context.md", role="memory",
    ...
    for pattern in [f"_grimoire/_memory/agent-learnings/{agent_id}.md",
                     f"_grimoire/_memory/{agent_id}-learnings.md"]:
    ...
    # Failure museum
    fm = project_root / "_grimoire/_memory/failure-museum.md"
    if fm.exists():
        loads.append(FileLoad(
            path="_grimoire/_memory/failure-museum.md", role="memory",
            tokens=_read_tokens(fm),
        ))

    # Grimoire_TRACE (last 200 lines approximation)
    ...
    if trace.exists():
        try:
            ...
            recent = "\n".join(lines[-200:])
            loads.append(FileLoad(
                path="_grimoire-output/Grimoire_TRACE.md", role="trace",
...
class ContextGuard(GrimoireTool):
    """Context budget analyzer for Grimoire agents."""
