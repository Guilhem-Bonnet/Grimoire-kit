"""Path and template variable resolution for Grimoire projects.
...
class PathResolver:
    ...
    def resolve_path(self, raw: str, *, must_exist: bool = False) -> Path:
        ...
        Raises :class:`GrimoireConfigError` if *must_exist* is ``True``
        ...
        if must_exist and not resolved.exists():
            raise GrimoireConfigError(f"Path does not exist: {resolved}")
        return resolved
