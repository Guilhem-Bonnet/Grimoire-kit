...
> Auto-generated by Grimoire Kit v$version
> Archetype: $archetype | Memory: $backend | Language: $language
...
class ProjectScaffolder:
    ...
    def _plan_copilot_prompts(self, p: ScaffoldPlan) -> None:
        """Copy Grimoire workflow prompts to .github/prompts/."""
        prompts_src = self._framework / "copilot" / "prompts"
    ...
    def _plan_copilot_instruction_files(self, p: ScaffoldPlan) -> None:
        ...
        instructions_src = self._framework / "copilot" / "instructions"
        if not instructions_src.is_dir():
            return
        gh_instructions = self._target / ".github" / "instructions"
        v = self._tpl_vars()
        for instr_file in sorted(instructions_src.glob("*.instructions.md")):
            ...
            content = content.replace("{{language}}", v["language"])
            p.templates.append(TemplateRender(
                dst=dst,
                content=content,
                label=f".github/instructions/{instr_file.name}",
            ))
