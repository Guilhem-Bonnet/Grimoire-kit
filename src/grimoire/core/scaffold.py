...
class ProjectScaffolder:
    ...
    def _tpl_vars(self) -> dict[str, str]:
        ...
        memory_extra = ""
        uses_redis = self._uses_redis_short_term()
        short_term_backend = "redis" if uses_redis else "sqlite"
        redis_url = "redis://localhost:6379/0" if uses_redis else ""
        layer_profile = self._layer_profile()
        memory_layers = (
            f'  layer_profile: "{layer_profile}"\n'
            f'  short_term_backend: "{short_term_backend}"\n'
    ...
    def _selected_archetypes(self) -> tuple[str, ...]:
        return self._resolved.archetypes or (self._resolved.archetype,)

    def _uses_redis_short_term(self) -> bool:
        archetypes = set(self._selected_archetypes())
        return self._backend == "weaviate-server" or bool(
            archetypes & _REDIS_SHORT_TERM_ARCHETYPES,
        )

    def _layer_profile(self) -> str:
        archetypes = self._selected_archetypes()
        if "infra-ops" in archetypes:
            return "infra-ops"
        if "platform-engineering" in archetypes:
            return "platform-engineering"
        return "standard"

    def _memory_requirements(self) -> str:
        lines = [
            "# Grimoire Memory System — generated by grimoire init",
            "pyyaml>=6.0",
        ]
        if self._backend in {"qdrant-local", "qdrant-server"}:
            lines.extend([
                "qdrant-client>=1.7.0,<1.13",
                "sentence-transformers>=2.2.0",
            ])
        elif self._backend == "ollama":
            lines.append("qdrant-client>=1.7.0,<1.13")
        elif self._backend == "weaviate-server":
            lines.extend([
                "sentence-transformers>=2.2.0",
                "neo4j>=5.0",
            ])
        if self._uses_redis_short_term():
            lines.append("redis>=5.0.0")
        return "\n".join(lines) + "\n"
    ...
    def _plan_framework(self, p: ScaffoldPlan) -> None:
        ...
        elif self._uses_redis_short_term():
            src = fw / "memory" / "docker-compose.redis.tpl.yml"
            if src.is_file():
                p.copies.append(FileCopy(
                    src=src,
                    dst=self._target / "docker-compose.redis.yml",
                    label="memory/docker-compose.redis.yml",
                ))
    ...
    def _plan_templates(self, p: ScaffoldPlan) -> None:
        ...
        p.templates.append(TemplateRender(
            dst=self._memory_dir() / "requirements.txt",
            content=self._memory_requirements(),
            label="_grimoire/_memory/requirements.txt",
        ))
