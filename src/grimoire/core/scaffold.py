...
> Archetype: $archetype | Memory backend: $backend
...
_grimoire/
  _config/
    custom/
      agents/           <- Project AI agents
      prompt-templates/  <- Prompt templates
      workflows/         <- Custom workflows
      agent-base.md      <- Agent base protocol
      cc-verify.sh       <- Completion Contract verifier
    agent-manifest.csv   <- Agent registry
  _memory/
    shared-context.md    <- Shared context across agents
    config.yaml          <- Memory configuration
    decisions-log.md     <- Architectural decisions
    failure-museum.md    <- Resolved error catalogue
_grimoire-output/
  .runs/                 <- Work sessions
project-context.yaml     <- Global project configuration
```
...
2. **Completion Contract**: Before each commit, verify with `bash _grimoire/_config/custom/cc-verify.sh`
3. **Context**: Load `_grimoire/_memory/shared-context.md` for project context
4. **Decisions**: Log architectural decisions in `_grimoire/_memory/decisions-log.md`
5. **Errors**: Document resolutions in `_grimoire/_memory/failure-museum.md`
"""
...
class ProjectScaffolder:
    ...
    def _plan_copilot_instructions(self, p: ScaffoldPlan) -> None:
        ...
        # Build agents table from planned copies
        all_agents = self._list_planned_agents(p)
        lines = ["| Agent | File |", "|-------|------|"]
        for label in all_agents:
            parts = label.split("/")
            name = parts[-1] if len(parts) > 1 else parts[0]
            lines.append(f"| {name} | _grimoire/_config/custom/agents/{name}.md |")
        agents_table = "\n".join(lines) if all_agents else "(none)"
