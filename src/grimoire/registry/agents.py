...
class AgentDef:
    ...
    def load_persona(self) -> str:
        ...
        if not self.path.is_file():
            raise GrimoireAgentError(f"Agent file not found: {self.path}")
...
class ArchetypeDNA:
    ...
    def from_yaml(cls, dna_path: Path) -> ArchetypeDNA:
        ...
        if not dna_path.is_file():
            raise GrimoireRegistryError(f"DNA file not found: {dna_path}")
        try:
            ...
            try:
                ...
                raise GrimoireRegistryError(f"Cannot parse DNA '{dna_path}': {exc}") from exc
        except Exception as exc:
            raise GrimoireRegistryError(f"Cannot parse DNA '{dna_path}': {exc}") from exc

        if not isinstance(raw, dict):
            raise GrimoireRegistryError(f"DNA file is not a YAML mapping: {dna_path}")
