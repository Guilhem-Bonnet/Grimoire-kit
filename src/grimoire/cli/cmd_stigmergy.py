...
def emit(
    ...
    stig.log_event(root, "emit", ptype=p.pheromone_type, location=p.location,
                   emitter=p.emitter)
...
def amplify(
    ...
    if p is None:
        ...
        raise typer.Exit(1)
    stig.save_board(root, board)
    stig.log_event(root, "reinforce", ptype=p.pheromone_type,
                   location=p.location, emitter=agent)
...
def resolve(
    ...
    if p is None:
        ...
        raise typer.Exit(1)
    stig.save_board(root, board)
    stig.log_event(root, "resolve", ptype=p.pheromone_type,
                   location=p.location, emitter=agent)
...
def install_hooks(
    ...
    root = _root(project_root)
    try:
        note = sh.install_hooks(root)
    except FileNotFoundError as exc:
        console.print(f"[red]✗[/red] {exc}")
        raise typer.Exit(1) from exc
    feat.set_enabled(root, "stigmergy-hooks", True)
    console.print("[green]✓[/green] Hooks stigmergiques installés dans "
                  f"[cyan]{root / '.github' / 'hooks'}[/cyan]")
...
def uninstall_hooks(
    ...
    feat.set_enabled(root, "stigmergy-hooks", False)
