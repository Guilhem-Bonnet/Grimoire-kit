"""Memory Lint — cross-file coherence analysis for BMAD memory.

Detects contradictions, duplicates, orphan decisions, stale entries,
and chronological inconsistencies in the ``_bmad/_memory`` directory.
...
def _parse_trace(path: Path) -> list[tuple[str, str]]:
    """Parse BMAD_TRACE.md into (date, text) entries."""
    try:
        content = path.read_text(encoding="utf-8")
    except OSError:
        return []
...
def collect_memory_files(project_root: Path) -> list[MemoryFile]:
    ...
    if trace.exists():
        ...
        if entries:
            files.append(MemoryFile(
                path="BMAD_TRACE.md", kind="trace", entries=tuple(entries),
            ))
