...
from bmad.tools._common import load_yaml, save_yaml
...
def detect_version(project_root: Path) -> str:
    ...
    # v3 has top-level 'bmad' key with 'version'
    if "bmad" in data and isinstance(data["bmad"], dict):
        return "v3"
...
def plan_upgrade(project_root: Path) -> UpgradePlan:
    ...
    plan.actions.append(UpgradeAction(
        kind="generate-file",
        description="Add 'bmad' section to project-context.yaml (v3 config)",
        target="project-context.yaml",
    ))

    # 2. Ensure v3 directories
    for d in ("_bmad", "_bmad/_memory", "_bmad-output",
              "_bmad/_config", "_bmad/_config/agents"):
        dp = project_root / d
        if not dp.is_dir():
            plan.actions.append(UpgradeAction(
                kind="create-dir",
...
def _generate_v3_section(v2_data: dict[str, Any]) -> dict[str, Any]:
    """Build the 'bmad' section from v2 config data."""
