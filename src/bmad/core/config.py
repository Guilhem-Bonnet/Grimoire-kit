"""Typed configuration for BMAD projects.
...
class MemoryConfig:
    ...
    def from_dict(cls, data: dict[str, Any]) -> MemoryConfig:
        ...
        return cls(
            backend=backend,
            collection_prefix=str(data.get("collection_prefix", "bmad")),
...
@dataclass(frozen=True, slots=True)
class BmadConfig:
    """Root BMAD project configuration.
    ...
    @classmethod
    def from_dict(cls, data: dict[str, Any]) -> BmadConfig:
        """Build a :class:`BmadConfig` from a parsed YAML dict.

        Raises :class:`BmadConfigError` on validation failures.
        """
        if not isinstance(data, dict):
            raise BmadConfigError("Config root must be a YAML mapping")

        raw_project = data.get("project")
        if not isinstance(raw_project, dict) or not raw_project.get("name"):
            raise BmadConfigError("Config must contain a 'project' section with a 'name' field")
    ...
    @classmethod
    def from_yaml(cls, path: Path) -> BmadConfig:
        """Load config from a YAML file.

        Raises :class:`BmadConfigError` if the file is missing, unreadable,
        or contains invalid YAML.
        """
        if not path.is_file():
            raise BmadConfigError(f"Config file not found: {path}")
        ...
        try:
            ...
            try:
                ...
                raise BmadConfigError(f"Cannot parse '{path}': {exc}") from exc
        except Exception as exc:
            raise BmadConfigError(f"Cannot parse '{path}': {exc}") from exc

        if raw is None:
            raise BmadConfigError(f"Config file is empty: {path}")

        return cls.from_dict(raw)

    @classmethod
    def find_and_load(cls, start: Path | None = None) -> BmadConfig:
        """Walk up the directory tree to find ``project-context.yaml``.

        Starts from *start* (default: cwd) and searches upward.
        Raises :class:`BmadConfigError` if no config file is found.
        """
