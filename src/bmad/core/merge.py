"""Non-destructive merge engine for BMAD projects.

Analyses a source BMAD installation against a target project to
...
class MergeEngine:
    ...
    ----------
    source :
        Directory containing the BMAD files to merge (e.g. archetype output).
    target :
    ...
    def __init__(self, source: Path, target: Path) -> None:
        ...
        if not self._source.is_dir():
            raise BmadMergeError(f"Source directory does not exist: {self._source}")
        if not self._target.is_dir():
            raise BmadMergeError(f"Target directory does not exist: {self._target}")
    ...
    def execute(
        ...
        *,
        ...
        if not dry_run and (created or skipped):
            log_path = self._target / ".bmad-merge-log.json"
            log_entry = {
    ...
    def undo(log_path: Path) -> list[str]:
        ...
        if not log_path.is_file():
            raise BmadMergeError(f"Merge log not found: {log_path}")
