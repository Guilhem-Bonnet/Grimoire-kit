...
- feat(init): étape Mémoire — profil le plus complet que la machine peut servir, santé vérifiée, fin du profil `--lite` (issue #616, PR 2/3 — suite de la PR 1 ci-dessus). L'audit avait montré que `grimoire init -y`/`up` sans `-a` retombait toujours sur `lexical`, même quand un Weaviate détecté sur `localhost:8080` aurait permis mieux, et que le seul mécanisme d'activation (`grimoire memory up --profile standard --apply`) retombait lui aussi en `lexical` faute de service serveur — aucun chemin ne donnait de vecteurs locaux sans Docker.
  ...
  - **Étape Mémoire de `grimoire init`, express et interactive** (`src/grimoire/cli/cmd_init.py`) : le chemin express (`-y`/non-TTY) applique désormais le profil le plus riche *privé et sans consentement à démarrer quoi que ce soit* — `standard` (Qdrant embarqué) avec un moteur d'embedding local installé, `lexical` sinon — jamais un service détecté (issue #496 intacte : détecté = suggéré, jamais câblé). `complet` reste nommé, avec sa commande d'activation, dès que Docker répond. Nouveau drapeau `--memory-stack up` : consentement explicite pour démarrer réellement la pile Docker dès l'`init` (express ou interactif), sans quoi aucun conteneur n'est jamais lancé. L'étape se termine par une vérification de santé structurelle (jamais un chargement de modèle d'embedding — ce coût reste celui du premier usage réel, `grimoire memory status`) reprise dans le rapport (`Memory:`, raison, commande pour monter d'un profil) et en JSON (`memory_profile`).
  ...
  - Tests rouge-avant : `tests/unit/tools/test_memory_setup.py` (repli `qdrant-local` sans serveur, `docker_daemon_reachable`, `local_embedding_available`, `recommend_profile`, `start_memory_stack` — Docker toujours simulé, jamais invoqué réellement), `tests/unit/cli/test_cmd_memory_ops.py` (`--start`), `tests/test_cmd_init.py`/`tests/unit/cli/test_app.py` (défaut express `qdrant-local`, jamais un service détecté, repli `lexical` sans capacité d'embedding, dépréciation `--lite`), `tests/unit/cli/test_init_lite_profile.py` (réécrit : `--lite` inerte + avertissement, `--memory-stack`), `tests/test_project_capabilities.py`/`tests/test_onboarding_panel.py` (comparaison au profil servable). `ruff check`/`mypy --strict` verts sur `src/grimoire/` en entier ; ratchet de taille inchangé (aucun fichier plafonné dépassé).
  - Rejeu réel : `grimoire init -y` sur trois projets jetables (Python nu, Node nu, dépôt vide, sans Docker simulé indisponible/disponible) — avant : `lexical` sans explication ; après : `standard` (Qdrant embarqué) avec raison nommée, ou `lexical` avec « no local embedding capability », panneau nommant le profil servi et la commande pour monter à `complet`. Rejeu en lecture seule sur la Forge (`grimoire memory status`) : profil `complet` déjà servi, `git status` identique avant/après (aucune écriture).
