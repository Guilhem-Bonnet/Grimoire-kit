...
- feat(init): onboarding en soixante secondes — détection des piles nues, discovery par défaut, next steps dynamiques (issue onboarding audit 2026-09-18, PR 1/3 — voir aussi #onboarding). Audit vécu sur trois projets jetables (Python nu, Node nu, dépôt vide, kit 3.57.0) : `grimoire init -y`/`up` sans `-a` retombait systématiquement sur `minimal` sans jamais le dire, le wizard interactif ne choisissait jamais de pattern par défaut, et le panneau « Next Steps » était identique sur les 6 runs testés. Décision produit de Guilhem, en cours de revue : `resolve()` ne rend plus jamais `minimal` automatiquement — `minimal` reste accessible uniquement via `-a minimal` explicite (démo/test).
  - **Détection des piles nues** (`src/grimoire/core/archetype_resolver.py`) : nouvelles `recommend_naked_stack()` (Python/Node/Rust/Go/dépôt vide → `platform-engineering` ou `web-app`, jamais `minimal`, avec raison nommée) et `weak_signal_suggestion()` (bundler dans `package.json`, Dockerfile nu, CI+tests → signal plus confiant, vérifié en premier). Nouveau champ `ResolvedArchetype.is_best_guess` : distingue un pick automatique (jamais confirmé par l'utilisateur) d'une correspondance de règle confiante ou d'un choix explicite.
  - **Discovery par défaut dans le wizard** (`src/grimoire/cli/cmd_init.py::_run_wizard`) : l'étape Archétypes défaut désormais sur `0` (discovery guidée, 3 questions oui/non) quand aucune règle n'a matché avec confiance, au lieu de `none` (→ minimal). Un pick automatique (`is_best_guess`) n'est plus jamais pré-rempli/affiché comme « détecté » dans la liste numérique — sinon le wizard confirmerait silencieusement une supposition. `_guided_discovery()` ne retombe plus jamais sur `minimal` en bout de chaîne (`platform-engineering`).
