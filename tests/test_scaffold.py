...
class TestProjectScaffolder:
    ...
    def test_agent_wrapper_references_internal_file(self, tmp_path: Path) -> None:
        ...
        s.execute(plan)
        content = (tmp_path / ".github" / "agents" / "concierge.agent.md").read_text()
        assert "_grimoire/kit/agents/concierge.md" in content

    def test_agent_wrapper_is_regenerated(self, tmp_path: Path) -> None:
        """Wrappers are generated pointers; they must track the agent set.

        A stale wrapper points at a path the current layout no longer uses,
        which breaks agent activation silently.
        """
        gh = tmp_path / ".github" / "agents"
        gh.mkdir(parents=True)
        existing = gh / "concierge.agent.md"
        existing.write_text("# Stale wrapper\n")
        s = _scaffolder(tmp_path)
        s.execute(s.plan())
        assert existing.read_text() != "# Stale wrapper\n"
