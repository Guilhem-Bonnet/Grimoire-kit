...
# --- P0.1 : le fail-closed ferme réellement ---------------------------------


def test_verify_detects_deleted_extra_artifact(tmp_path: Path) -> None:
    """An artifact activated by a need cannot be deleted unnoticed.

    ``setup_standard_profile`` records the complete artifact set — extras
    included — in ``standard-profile.yaml``.  Before this test, ``verify``
    recomputed the required set from the profile id alone and returned
    ``ok=True, missing=[]`` after the deletion.
    """
    setup_standard_profile(tmp_path, profile_id="starter", extra_artifacts=("evidence_gates",))
    gates = tmp_path / "_grimoire/standard/evidence-gates.yaml"
    ...
    assert _posix_path(str(Path("_grimoire/standard/evidence-gates.yaml"))) in {
        _posix_path(str(path)) for path in result.missing
    }


def test_verify_required_set_depends_on_needs_not_only_profile(tmp_path: Path) -> None:
    """Two projects on the same profile but different needs differ in requirements."""
    bare = tmp_path / "bare"
    with_extra = tmp_path / "with-extra"
    setup_standard_profile(bare, profile_id="starter")
    setup_standard_profile(with_extra, profile_id="starter", extra_artifacts=("evidence_gates",))

    bare_paths = {_posix_path(str(p)) for p in verify_standard_profile(bare).present}
    extra_paths = {_posix_path(str(p)) for p in verify_standard_profile(with_extra).present}

    assert bare_paths < extra_paths
    assert "_grimoire/standard/evidence-gates.yaml" in extra_paths - bare_paths


@pytest.mark.parametrize("profile_id", ["starter", "controlled", "orchestrated", "governed", "production"])
def test_gate_check_strict_exits_2_for_every_profile(tmp_path: Path, profile_id: str) -> None:
    """``--strict`` escalates to exit 2 on every profile, not only governed/production.

    The gate is about transitions, so the project must claim a lifecycle state for
    anything to be required: ``released`` demands an evidence pack, a decision trace
    and a compliance score that a freshly scaffolded project does not have.
    """
...
def test_standard_json_payloads_carry_a_versioned_schema(
    tmp_path: Path, argv: list[str], schema: str, keys: set[str]
) -> None:
    """A third-party CI parsing these payloads gets a versioned contract.

    The exact top-level key set is locked: adding or removing one is a
    contract change, not an implementation detail.
    """
    setup_standard_profile(tmp_path, profile_id="starter")
    runner = CliRunner()
    result = runner.invoke(app, ["-o", "json", *argv, str(tmp_path)])
