...
> **Menu du wizard** — `grimoire init` propose 6 points de départ : `minimal` (base) + `web-app`, **Platform & Infra** (`platform-engineering`), `creative-studio`, `game-dev`, `fix-loop`. Les sections ci-dessous documentent **tous** les archétypes, y compris ceux qui ne sont pas des choix de menu : `infra-ops` (auto-détecté pour Terraform/K8s/Ansible, replié sous « Platform & Infra »), `agentic-standard` (overlay `--archetype`), et les composants internes `meta`/`stack`/`features`.
