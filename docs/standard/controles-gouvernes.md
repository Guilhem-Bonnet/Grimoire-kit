...
| `prompt-injection-firewall` | controlled | Isolate external/untrusted content so it cannot override control instructions. | `prompt-firewall.yaml`, `tool-manifest.csv` | `firewall.isolation_disabled`, `firewall.override_allowed`, `firewall.untrusted_output_unwrapped`, `firewall.artifact_missing` |
