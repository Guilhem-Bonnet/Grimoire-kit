...
| `src/grimoire/core/scaffold.py` | 6 (4 + 2 aux alertes 599/600/601, PR #618, addendum ci-dessous) | `target`/`self._target` : dossier de destination `grimoire init`, ou projet déjà enregistré sélectionné depuis le cockpit (`cmd_cockpit.py` → `_resolve_project_path`, jamais construit depuis l'entrée) |
...
## Addendum — PR #618 (issue #616), alertes 599/600/601
...
`_detected_stack_skill_slugs()`, issue #616). Une troisième, alerte 601, est apparue après un commit
suivant de la même PR (le correctif du rust-gate, ci-dessous) qui a décalé ces lignes de un cran :
CodeQL n'a pas reconnu `existing.read_text(...)` à sa nouvelle position comme la même instance que
l'alerte 600 déjà dismissée et en a ouvert une nouvelle sur le même sink, au même endroit du code.
Dismissée avec la même justification, ci-dessous. Avant de les classer, lecture
...
premier lieu. Ces trois alertes (599, 600, 601 — la dernière rouverte sur le
même sink après un décalage de ligne, voir ci-dessus) rejoignent le lot des
faux positifs dismissés (classe (b), même ligne `scaffold.py` du tableau
agrégé, portée de 4 à 6 alertes couvertes — 601 ne s'ajoute pas au compte,
elle remplace 600 au même endroit) plutôt que d'attendre une fermeture qui
ne viendra pas. Dismiss via l'API, commentaire référençant ce paragraphe.
Si un futur commit décale ces lignes une fois de plus, s'attendre au même
symptôme (nouvelle alerte sur le même sink) et appliquer la même dismiss.
