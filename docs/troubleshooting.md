...
# <img src="../assets/icons/microscope.svg" width="32" height="32" alt=""> Dépannage — Grimoire Kit
...
> **Voir aussi** : [docs/vscode-setup.md](vscode-setup.md#5-modeles-et-rate-limits) pour la configuration complète des modèles.
...
> **Voir aussi** : [docs/vscode-setup.md](vscode-setup.md#7-reseau-et-vpn) pour la configuration réseau complète.
...
## <img src="../assets/icons/network.svg" width="28" height="28" alt=""> 18. API Observatory — erreurs 400, 403, 404

**Symptôme** : l'interface Observatory ou un appel `curl` retourne `400`, `403` ou `404` sur les endpoints `/api/agent-config/*`.

**Diagnostic rapide** :

```bash
# Vérifier les drapeaux runtime de l'API
curl -sS http://127.0.0.1:8082/api/agent-config | jq '{version: .config.version, commit_required, read_only}'

# Lire les derniers événements d'audit
tail -n 20 _grimoire-output/.agent-config-audit.jsonl
```

**Causes et correctifs** :

- `400 invalid JSON` : payload mal formé
  - Envoyer un JSON valide avec `Content-Type: application/json`
- `400 commit message required` : serveur lancé avec `--commit-required`
  - Ajouter le champ `commit_message` dans `POST /api/agent-config/apply`
- `403 mutations disabled in read-only mode` : serveur lancé avec `--read-only`
  - Relancer sans `--read-only` pour autoriser `apply` et `rollback`
- `404 backup not found` : rollback demandé sans backup disponible
  - Faire un `apply` avant rollback, ou restaurer le fichier de config depuis Git
- `404 endpoint not found` : URL API incorrecte
  - Utiliser exactement `/api/agent-config`, `/diff`, `/apply`, `/rollback`

**Validation fonctionnelle** :

```bash
curl -sS http://127.0.0.1:8082/api/agent-config
```

La réponse doit inclure `ok: true` et refléter les drapeaux `commit_required` et `read_only`.

> **Voir aussi** : [Guide de l'API Observatory](observatory-api.md)
