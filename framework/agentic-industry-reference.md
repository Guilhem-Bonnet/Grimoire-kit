...
| ELSS, journal d'événements partagé (`event-log-shared-state.md`) | Checkpointers LangGraph, task ledger Magentic, mailbox agent teams | **Export en spans OTel GenAI livré** (`TraceLedger.export_otel_jsonl`, un span `invoke_agent` par trace plus un `execute_tool` par appel d'outil, format semconv exact — #322, 2026-09-08, fusionné après la rédaction de cette référence). Non consommé : `grimoire task trace-export --format otel` reste une commande manuelle, rien ne l'alimente en continu ni ne le corrèle aux refus de policy ou aux checkpoints du kernel (issue #139, ouverte, portée exacte : timeline unifiée par tâche) | à combler |
...
| Pont MCP du kit (`grimoire` serveur MCP) | MCP 2026-07-28 stateless, MRTR, `server/discover`, headers `Mcp-Method` | Le kit cible une révision antérieure ; planifier la migration avant le retrait de Roots et Sampling (2027-07-28). Revérifié le 2026-09-12 (`src/grimoire/mcp/server.py`) : les commentaires du code citent toujours 2025-03-26 (annotations d'outil) et 2025-06-18 (`structuredContent`) comme bornes basses du SDK, intact | à combler |
| Observabilité cockpit, `observability-policy.yaml` | OTel GenAI spans et métriques, Langfuse, Phoenix | **Émission livrée** : `otel_conventions.py` définit le vocabulaire semconv exact (`gen_ai.provider.name`, spans `invoke_agent`/`execute_tool`) et `TraceLedger.export_otel_jsonl` (#322) les produit réellement. **Consommation intacte** : `/api/otel` existe (`forge_routes.py`) mais aucune page de `web/` ne l'appelle ; `web/observability.html` reste sur l'ancien chemin `framework/tools/observatory.py` (issue #139, ouverte, portée = timeline par tâche corrélant refus de policy, checkpoints et traces) | à combler |
...
1. Brancher le cockpit sur l'export OTel déjà livré (#322) : `/api/otel` existe et personne ne l'appelle ; corréler via `task_id` sur `GrimoireEvent` (#139, ouverte — c'est déjà le ticket qui porte cette portée).
2. Faire courir pass^k en continu et instrumenter le coût par tâche résolue directement dans les gates de preuve, pas seulement en campagne ponctuelle ou en rejeu manuel (#307).
...
6. Réparer #427 (un override d'agent en copie intégrale ne reçoit plus jamais les mises à niveau du kit, sans aucun signal de `doctor`) — promesse de personnalisation de la doctrine des artefacts non tenue par le code, distincte d'un écart face à l'industrie.
