# Agentic Compliance Declaration

## Declaration

- Project:
- Declared profile: `starter | controlled | orchestrated | governed | production`
- Standard reference:
- Declaration owner:
- Review cadence:

## Profile coverage

| Requirement or capability | Status | Evidence | Gap |
|---|---|---|---|
| Workflow State Engine | unknown |  |  |
| Advanced Context Orchestrator | unknown |  |  |
| Evidence-Gated Workflow FSM | unknown |  |  |
| Agent Telemetry Plane | unknown |  |  |
| Tool Blast-Radius Limiter | unknown |  |  |
| Knowledge Base Indexer | unknown |  |  |
| LLM Provider Abstraction | unknown |  |  |

## Provider compatibility

| Provider | Status | Allowed use | Data policy |
|---|---|---|---|
| GitHub Copilot | unknown |  |  |
| OpenAI Codex/OpenAI | unknown |  |  |
| Anthropic Claude | unknown |  |  |
| Google Gemini | unknown |  |  |
| Local/open-weight | unknown |  |  |

## Non-conformities

| Item | Severity | Remediation | Owner |
|---|---|---|---|
|  |  |  |  |

## Statement

This declaration describes the Grimoire Kit implementation state for a selected profile. It does not replace the upstream normative corpus.
