# Agentic Mission Brief

## Identity

- Project:
- Owner:
- Selected profile: `starter | controlled | orchestrated | governed | production`
- Upstream standard reference:
- Date:

## Scope

- In scope:
- Out of scope:
- Critical assets:
- Risk level: `low | medium | high | critical`

## Flow objectives

| Objective | Expected outcome | Evidence required |
|---|---|---|
|  |  |  |

## Mandatory capabilities

| Capability | Required? | Grimoire artifact | Notes |
|---|---:|---|---|
| Workflow State Engine | yes | Task envelope |  |
| Advanced Context Orchestrator | profile-dependent | Task envelope, context policy |  |
| Knowledge Base Indexer | profile-dependent | Knowledge source registry | Not memory, not session context |
| LLM Provider Registry | profile-dependent | Provider registry | Provider-first routing |
| Evidence-Gated Workflow | yes | Evidence pack |  |

## Governance assumptions

- Approved tools:
- Writable paths:
- External services:
- Data classes allowed in prompts:
- Data classes forbidden in prompts:

## Known deviations

| Deviation | Reason | Expiry or review trigger |
|---|---|---|
|  |  |  |
