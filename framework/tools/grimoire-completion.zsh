#!/usr/bin/env zsh
# grimoire-completion.zsh — BM-38 : Shell completions pour grimoire-init.sh
#
# Installation (zsh) :
#   echo "source /path/to/grimoire-kit/framework/tools/grimoire-completion.zsh" >> ~/.zshrc
#   source ~/.zshrc
#
# Installation (bash) :
#   # Le fichier bash-completion est à la fin de ce script
#   echo "source /path/to/grimoire-kit/framework/tools/grimoire-completion.bash" >> ~/.bashrc

# ── Détection du répertoire du kit ──────────────────────────────────────────
_Grimoire_KIT_DIR="${Grimoire_KIT_DIR:-$(dirname "$(dirname "$(dirname "${(%):-%x}")")")}"

# ── Listage dynamique des archétypes ────────────────────────────────────────
_grimoire_archetypes() {
    local archetypes_dir="${_Grimoire_KIT_DIR}/archetypes"
    local -a archetypes
...
_grimoire_checkpoints() {
    find "${PWD}/_grimoire-output/.runs" -name "state.json" -exec grep -l '"checkpoint_id"' {} \; 2>/dev/null \
        | xargs -I{} sh -c 'grep "checkpoint_id" "$1" | grep -oP "[a-f0-9]{6}"' _ {} 2>/dev/null
}

# ── Listage des branches de session ─────────────────────────────────────────
_grimoire_branches() {
    ls -d "${PWD}/_grimoire-output/.runs"/*/ 2>/dev/null | xargs -I{} basename {}
}

# ── Completion principale zsh ────────────────────────────────────────────────
_grimoire_init_complete() {
    local state
    local -a subcommands
    ...
    subcommands=(
        ...
        'trace:Gérer le Grimoire Trace audit trail'
        'doctor:Diagnostic et health check du kit'
        'validate:Valider des fichiers DNA archétype'
        'changelog:Générer CHANGELOG.md depuis Grimoire_TRACE'
        ...
        'migrate:Migration cross-projet d'artefacts Grimoire'
    ...
    case "$state" in
        ...
        archetypes)
            local -a arch_list
            arch_list=($(_grimoire_archetypes))
            ...
            case "${line[1]}" in
                ...
                install)
                    _arguments \
                        ...
                        '--target[Répertoire Grimoire cible]:directory:_directories' \
                ...
                trace)
                    _arguments \
                        ...
                        '--agent[Filtrer par agent]:agent:(dev qa architect pm sm)' \
                ...
                forge)
                    _arguments \
                        ...
                        '--from-gap[Générer depuis les lacunes détectées dans Grimoire_TRACE]' \
                ...
                evolve)
                    _arguments \
                        ...
                        '--trace[Fichier Grimoire_TRACE source]:trace_file:_files' \
...
compdef _grimoire_init_complete grimoire-init.sh
compdef _grimoire_init_complete grimoire-init

# ────────────────────────────────────────────────────────────────────────────
# Fichier bash-completion séparé (créer comme grimoire-completion.bash)
# ────────────────────────────────────────────────────────────────────────────
# #!/usr/bin/env bash
# # Installation : source /path/to/grimoire-completion.bash
#
# _grimoire_init_bash() {
...
#
#     case "$prev" in
#         grimoire-init.sh|grimoire-init)
...
# }
#
# complete -F _grimoire_init_bash grimoire-init.sh grimoire-init
