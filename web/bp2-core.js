...
(async function () {
  ...
  /* ══ Sections déclaratives de l'inspecteur ══ — moteur dans bp2-sections.js */
  const S = () => window.BP2Sections;
  const gateSectionHtml = n => (S() ? S().gateSectionHtml(n) : '');
  const bindGateSection = (p, n) => { if (S()) S().bindGateSection(p, n); };
  const contextSectionHtml = n => (S() ? S().contextSectionHtml(n) : '');
  const bindContextSection = (p, n) => { if (S()) S().bindContextSection(p, n); };
  ...
  /* ══ Lexique / aide ══ — moteur dans bp2-lexicon.js */
  const openLex = k => (window.BP2Lexicon ? BP2Lexicon.openLex(k) : null);
  const lexify = t => (window.BP2Lexicon ? BP2Lexicon.lexify(t) : String(t == null ? '' : t));
  ...
  const core = {
    on, emit, state: () => state, bpId: () => bpId, path: () => curPath.slice(),
    CTX_TIERS, CTX_STRATEGIES, CTX_ISOLATIONS,
    hasVocab: () => !!VOCAB,
    ctxOf,
